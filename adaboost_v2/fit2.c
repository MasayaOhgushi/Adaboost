...
void fit2(struct rec *t_func,struct data *L_data){
	
	char *eva;	
	int i,j,k=0,Nm,Np,l=0;
	double *w;
...
// Evaluation
	if(k>=L_data[0].num-1)
	{
        ...
        for(l=0;l<L_data[0].num;l++)
		{
//		sprintf(eva,"%d %lf %lf >>e_test\n",s_flag[l],sepa[l],alpha[l]);
		printf("%d %lf %lf\n",s_flag[l],sepa[l],alpha[l]);
//		system(eva);
//		fprintf(fp100,"%d %lf %lf\n",s_flag[l],sepa[l],alpha[l]);
		}
